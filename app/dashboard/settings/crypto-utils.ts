...
/**
 * Generates a cryptographically secure random API key prefixed with 'rs_'.
 */
export function generateRandomApiKey(): string {
  const bytes = crypto.getRandomValues(new Uint8Array(24));
  return 'rs_' + bytesToHex(bytes);
}

/**
 * Computes the SHA-256 hash of an API key for database storage/comparison.
 */
